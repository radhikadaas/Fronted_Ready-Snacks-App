...
export default function Payment() {
  ...
  async function placeOrder() {
    ...
    const razorpayOrder = await fetch("http://localhost:5000/create-order", {
      method: "POST",
    ...
    const options = {
      key: import.meta.env.VITE_RAZORPAY_KEY_ID, // public test key
      ...
      handler: async function (response) {
        ...
        // 5️⃣ CREATE ORDER IN SUPABASE (AFTER PAYMENT SUCCESS)
        const orderPayload = {
          user_id: user.id,
        ...
        <div className="bg-white/10 backdrop-blur-lg p-8 rounded-2xl border border-white/10 shadow-xl space-y-8">
              ...
              <div className="rounded-2xl border border-white/10 bg-linear-to-br from-gray-800/70 to-gray-900/80 p-6 space-y-4 shadow-inner">
                ...
                <div className="rounded-lg bg-white/5 border border-white/10 p-4 text-sm text-gray-300">
                  <p className="mb-2 font-medium text-indigo-200">
                    Don’t worry — we’ll confirm your location on Chat/Call. ☎️
                  </p>
          ...
          <div>
            ...
            {cart.map((item) => {
              ...
              return (
                <div key={item.id} className="bg-white/5 p-4 rounded-xl mb-3">
                  {/* 🔥 DISCOUNT BADGES */}
                  {hasDiscount && (
                    <div className="flex gap-2 mb-2">
                      <ProductBadges product={item} variant="payment" />
                    </div>
                  )}
          ...
          >
            {isLoading ? "Opening Payment..." : "Pay Securely"}
          </button>
        </div>
      </div>
    </div>
